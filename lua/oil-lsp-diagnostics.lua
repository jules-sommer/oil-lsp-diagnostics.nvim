...
local function apply_padding(text)
  local left_padding = string.rep(current_config.padding.left.char or "", current_config.padding.left.amount or 0)
  local right_padding = string.rep(current_config.padding.right.char or "", current_config.padding.right.amount or 0)
  ...
  for n = 1, vim.api.nvim_buf_line_count(buffer) do
    ...
    if entry then
      if is_dir then
        ...
        diagnostics = file_buf and get_diagnostics_summary(file_buf, false) or get_diagnostics_summary(file_path, false)
      end
    end
    ...
    if diagnostics then
      ...
      for _, key in ipairs({ "error", "warn", "info", "hint" }) do
        ...
        if count and count > 0 then
          ...
          table.insert(virt_text, { apply_padding(text), color })
        end
      end
      ...
      if #virt_text > 0 then
        ...
        if current_config.alignment == "right" then
          extmark_opts.virt_text_pos = "right_align"
        elseif current_config.alignment == "left" then
          extmark_opts.virt_text_pos = "eol"
        end

        vim.api.nvim_buf_set_extmark(buffer, namespace, n - 1, 0, extmark_opts)
      end
    end
  end
end
